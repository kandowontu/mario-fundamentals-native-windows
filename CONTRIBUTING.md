...
- Keep reverse-engineering claims tied to a resource ID, CODE offset, trace, or repeatable original
  behavior.
...
That command builds a static release, runs the executable self-test, validates the PE dependency
boundary, performs the full byte-preservation audit when the ignored extraction manifest is
present, and writes the release checksum.
