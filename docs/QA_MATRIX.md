...
| Port scenes retain original whole-frame geometry | Forty-seven independent comparisons cover Macintosh publisher/title states, the black-cast board silhouette, four selected menu poses, stable gameplay geometry in both editions, seven Macintosh and all five DOS game-intro instants, first-use character/name panels, Backgammon setup, Go Fish question/transfer states, both Dominoes score-portrait registrations, and focused Yacht dice/marker/actor/gesture/cup regions. A 219-file inventory rejects unaccounted retained captures. `docs/VISUAL_REFERENCE_AUDIT.md` records the method and measured margins. | Automated pass |
