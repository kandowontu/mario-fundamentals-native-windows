...
| Port scenes retain original whole-frame geometry | Thirty-nine independent comparisons cover Macintosh publisher/title states, stable gameplay geometry in both editions, seven Macintosh and all five DOS game-intro instants, first-use character/name panels, Go Fish hand transfers, both Dominoes score-portrait registrations, and focused Yacht actor/gesture/cup regions. `docs/VISUAL_REFERENCE_AUDIT.md` records the method and measured margins. | Automated pass |
...
isolated-runtime checks. It now requires exactly 219 fresh
Macintosh frames and 220 fresh DOS frames at their native logical dimensions. Any missing, stale,
