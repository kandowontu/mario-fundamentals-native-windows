...
shortcut while any selected-game title is active. `tools/verify_mac_game_intro_input.py` reads the
five original CODE payloads rather than trusting this prose or a native lookup table: it pins each
full resource hash, decodes the event-5/event-6 `BEQ.W` targets, and checks the target signatures.
The release gate separately sends all ten routes through an invisible packaged HWND, including
client-to-logical mouse conversion and the paired `WM_CHAR` generated by a printable skip key.

Yacht's scorecard-board opening is a different controller from that skippable title. CODE 18's
second export dispatches event 6 from `$3EA` to `$6F0`, which gates and hit-tests the ordinary
gameplay controls; it never posts export 1's `$228` title-completion callback. The `$AD2` opening
keeps those controls locked while Mario says index 11 ("Good luck"), waits five passes, says index
18 ("I go first"), and optionally follows `$CF8` through index 20 before the first roll. A click on
the visible scorecard board during those lines therefore does not skip in vanilla. The native
self-test clicks the live roll control during this state and requires the opening, roll count, and
movie ownership to remain unchanged.
