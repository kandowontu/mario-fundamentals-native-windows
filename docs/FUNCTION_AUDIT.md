...
| Sound Manager sampled playback | `snd ` resources, CODE 1 `$B22`/`$CAA`, and sound traps | All 313 resources validated; in-memory WAVE construction; concurrent WinMM `waveOut` voices; all 666 movie cue events audited; direct startup and per-game effect calls mapped; distinct speech/direct-effect lifetimes preserve the source busy gate, including snd 5003's 105 ms suppression across 33 ms menu-pointer steps |
...
- The silent presentation gate regenerates 230 Macintosh and 231 DOS logical-resolution frames,
