...
| Sound Manager sampled playback | `snd ` resources, CODE 1 `$A18`/`$A44`/`$B22`/`$CAA`, and sound traps | All 313 resources validated; in-memory WAVE construction; concurrent WinMM `waveOut` voices; all 666 movie cue events audited. A fail-closed instruction scan pins all 65 absolute sample calls—45 tracked `$A18` replacements and 20 `$CAA` direct effects—plus ten dynamic arguments and both immediate-ID sets. Native startup/UI/game routes preserve that lifetime split, including tracked Backgammon/Go Fish/Yacht calls, the tracked Checkers wipe versus concurrent movement, and Dominoes' mixed routes. Distinct tracked/direct lifetimes union for the shared source busy query. All fifteen `$B22` callers are dispositioned, including snd 5003's 105 ms suppression across 33 ms menu-pointer steps, title/menu launch waits, progressive Dominoes deal/result/reset gates, and Go Fish's standalone-26015 continuation. |
...
| Yacht rules/state/AI | CODE 18 | Native engine with source scorecard order and scoring values, `$2F22`'s twelve 96x20 right-player score controls and twenty-pixel score placement, exact index-11/18 Mario-first opening plus `$CF8`'s optional index-20 branch, source-registered Macintosh dialogue head/torso actors and a two-cel-composed movie-6021 idle actor, both pre-roll movie-6021 gestures at `(15,-1)` (wide bounds `(99,18)-(337,212)`), and pipe-roll movie 6020 as the sole large-cup owner. `$215C` creates only Pak 6010 frames 13/14 as the small remaining-roll controls, and neither it nor `$2F22` creates frame 12 as an idle large cup; the renderer therefore has no large cup before a roll or after movie 6020 ends. Executable Macintosh/DOS pre-roll-to-roll checks cover the visual transition and all five sequential settle passes, while all 960 Macintosh/1,200 DOS movie instants contain exactly one animated movie cel. The small cup-shaped controls remain intentionally visible: three before the first roll and two once it starts. CODE 18 export 1's Yacht title is click-skippable, but export 2's event-6 branch at `$3EA` reaches `$6F0`'s gameplay controls rather than a completion post; the visible-board "Good luck" / "I go first" opening therefore keeps vanilla's input lock. The controller retains exact 30000/5000 die buckets, `$2B42`/`$2C40`'s exact white-reroll/red-held lanes, same-pass held-die skipping, progressive white-die settling, legal all-five rerolls, and `$215C`'s markers decremented at roll start. The recovered `$3596` adviser includes its internal category order, immediate-bank priorities, triple/straight/pair retention, duplicate handling, final-category ties, and low-line sacrifice rule. All thirteen direct `$352C` sites are mapped: module-load joke seeding, opening, filled-line feedback, two player-idle gates, idle-line choice, score reaction, computer thinking, conditional reroll speech, five dice, and both outcome pools. `$190`'s four thinking/reaction selectors use their original lazy shuffles. The `$12A0`/`$142E` idle controller waits 80 source ticks, uses its 75/25 prompt/joke split, varies the prompt with rolls remaining, and cycles the recovered Pizza/Jamaica/Giovanni/Yucca five-part conversations with five-tick gaps. `$19E2` preserves source indices 42, 44/45, and 39/43 for tie/player/Mario outcomes, the player-only 6022 celebration, common index-53 replay question, and 24-tick scorecard clear. Input geometry, opening input lock, dialogue/RNG, strategy, actor/cup-layer presentation, and all three result branches have executable regression coverage. |
...
- The nine authored PICT help pages used by the original are rendered through the source QuickDraw environment, cleaned only of seven captured host-pointer artifacts by a source/output-hash-pinned tool, embedded as 486x350 modal panels, and navigated with their original page chrome and buttons.
