...
| Yacht rules/state/AI | CODE 18 | Native engine with source scorecard order and scoring values, `$2F22`'s twelve 96x20 right-player score controls and twenty-pixel score placement, exact index-11/18 Mario-first opening plus `$CF8`'s optional index-20 branch, pipe-roll movie with the stationary frame-12 cup suppressed through its five sequential settle passes in both editions and at most one live large-cup cel at every movie tick, exact 30000/5000 die buckets, `$2B42`/`$2C40`'s exact white-reroll/red-held lanes, same-pass held-die skipping, progressive white-die settling, legal all-five rerolls, and `$215C`'s green/yellow remaining-roll markers decremented at roll start. The two small cup-shaped controls remain intentionally visible as the source remaining-roll markers. The recovered `$3596` adviser includes its internal category order, immediate-bank priorities, triple/straight/pair retention, duplicate handling, final-category ties, and low-line sacrifice rule. All thirteen direct `$352C` sites are mapped: module-load joke seeding, opening, filled-line feedback, two player-idle gates, idle-line choice, score reaction, computer thinking, conditional reroll speech, five dice, and both outcome pools. `$190`'s four thinking/reaction selectors use their original lazy shuffles. The `$12A0`/`$142E` idle controller waits 80 source ticks, uses its 75/25 prompt/joke split, varies the prompt with rolls remaining, and cycles the recovered Pizza/Jamaica/Giovanni/Yucca five-part conversations with five-tick gaps. `$19E2` preserves source indices 42, 44/45, and 39/43 for tie/player/Mario outcomes, the player-only 6022 celebration, common index-53 replay question, and 24-tick scorecard clear. Input geometry, dialogue/RNG, strategy, cup-layer presentation, and all three result branches have executable regression coverage. |
...
- The silent presentation gate regenerates 200 Macintosh and 210 DOS logical-resolution frames,
  ...
  and Yacht score/selection/victory states, both Yacht roll/settle sequences, and DOS menu/help
  context screens. Sixteen independent original-output comparisons include complete source-timed
  frames for all five DOS intros, so their registered foreground actors are checked as well as the
  stable boards, tables, scorecards, and window chrome.
